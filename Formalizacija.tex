...
Druga tehnika je korišćenje operatora proširenja $\nabla : L \rightarrow L$ takvog da je $x, y \sqsubseteq x \nabla y$ za sve $x, y$ i pomoću koga se za bilo koji rastući niz $(y_{n})_{n}$ može napraviti niz $$(x^{\prime}_{n})_{n} \quad \text{gde je} \quad x^{\prime}_{0} = y_{0} \quad \text{i} \quad x^{\prime}_{n-1} \nabla y_{n} $$ takav da konvergira u konačnoj broju koraka. \\
